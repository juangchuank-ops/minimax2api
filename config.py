...
Sources (priority high → low):
  ...
  2. config.json file
  3. Built-in defaults
  4. Auto-detection (region-based base URL)
...
logger = logging.getLogger("minimax2api.config")
...
class Account:
    ...
    is_active: bool = True
    request_count: int = 0
    failure_count: int = 0           # consecutive failures for backoff calculation
    last_used: float = 0.0
...
class ConfigManager:
    ...
    def _load(self):
        ...
        try:
            ...
            print(f"[Config] JSON decode error: {exc}")
            self.config = Config()
            self._save()
        except OSError as exc:
            print(f"[Config] File I/O error: {exc}")
            self.config = Config()
            self._save()
        except Exception as exc:
            print(f"[Config] Unexpected load error: {exc}")
            self.config = Config()
            self._save()
    ...
    def _apply_env(self):
        with self._lock:
            ...
            if v := os.environ.get("MINIMAX_BASE_URL", "").strip():
                ...
                try:
                    from region_detect import get_recommended_base_url
                    recommended = get_recommended_base_url()
                    if recommended != self.config.minimax_base_url:
                        logger.info("自动切换到推荐的 base URL: %s", recommended)
                        self.config.minimax_base_url = recommended
                except ImportError:
                    logger.warning("区域检测模块未找到，使用默认 base URL")
                except Exception as e:
                    logger.warning("区域检测失败: %s，使用默认 base URL", e)
    ...
    def update_config(self, new_cfg: dict):
        with self._lock:
            # Validate proxy_api_keys
            proxy_keys = new_cfg.get("proxy_api_keys", self.config.proxy_api_keys)
            if not proxy_keys or not isinstance(proxy_keys, list):
                raise ValueError("proxy_api_keys must be a non-empty list")
            ...
            for i, acc in enumerate(accounts):
                if not isinstance(acc, dict):
                    raise ValueError(f"Account {i} must be a dict")
                if not acc.get("name"):
                    raise ValueError(f"Account {i} missing 'name' field")
                if acc.get("auth_mode") not in ("api_key", "token", None):
                    raise ValueError(f"Account {i} has invalid auth_mode")
