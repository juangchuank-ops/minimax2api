...
def _mark_used(acct: Account):
    ...
    acct.failure_count = 0
    # Re-enable account if it was disabled
    if not acct.is_active and not acct.on_cooldown:
        acct.is_active = True
        logger.info("Account '%s' re-enabled after successful request", acct.name)


def _mark_failed(acct: Account, status: int):
    """Apply exponential-backoff cooldown on error."""
    acct.failure_count = getattr(acct, 'failure_count', 0) + 1
    backoff = min(60 * (2 ** min(acct.failure_count, 5)), 600)
    acct.cooldown_until = time.time() + backoff
    acct.is_active = False
    logger.warning("Account '%s' cooldown %ds after HTTP %d (failure #%d)", 
                   acct.name, backoff, status, acct.failure_count)
    ...
    for field in ("temperature", "top_p", "max_tokens", "stream",
        ...
        if field in params and params[field] is not None:
            if field == "temperature":
                temp = float(params[field])
                if not (0.0 <= temp <= 2.0):
                    logger.warning("Temperature %.2f out of range, clamping to [0.01, 1.0]", temp)
                payload["temperature"] = max(0.01, min(1.0, temp))
            elif field == "top_p":
                top_p = float(params[field])
                if not (0.0 <= top_p <= 1.0):
                    logger.warning("top_p %.2f out of range, clamping to [0.0, 1.0]", top_p)
...
async def _execute_request(
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        try:
            ...
            logger.warning("Request timeout for account '%s': %s", acct.name, e)
            _mark_failed(acct, 504)
            
            # Suggest alternative URL if timeout
            _suggest_url_switch(acct.base_url)
            
            raise HTTPException(
                status_code=504,
                detail={"error": {"message": "MiniMax 请求超时", "type": "timeout_error"}},
            )
        except httpx.ConnectError as e:
            logger.warning("Connection error for account '%s': %s", acct.name, e)
            _mark_failed(acct, 503)
            
            # Suggest alternative URL if connection failed
            _suggest_url_switch(acct.base_url)
            
            raise HTTPException(
                status_code=503,
                detail={"error": {"message": f"无法连接到 MiniMax: {str(e)}", "type": "connection_error"}},
            )
...
def _suggest_url_switch(current_url: str):
    """Log suggestion to switch base URL if connection fails."""
    try:
        from region_detect import suggest_alternative_url
        alt_url = suggest_alternative_url(current_url)
        if alt_url:
            logger.warning(
                "提示: 当前 base URL (%s) 连接失败。"
                "如果你在中国国内，请尝试使用 api.minimaxi.com；"
                "如果在海外，请使用 api.minimax.io。"
                "建议的替代 URL: %s",
                current_url, alt_url
            )
    except Exception:
        pass  # Ignore if region_detect not available
...
async def proxy_chat(model: str, messages: list, params: dict, proxy_key: str) -> dict:
    ...
    if _is_web_agent(acct):
        ...
        try:
            ...
            logger.error("Web agent HTTP error: %s", e, exc_info=True)
            _mark_failed(acct, 503)
            raise HTTPException(
                status_code=503,
                detail={"error": {"message": f"网页代理请求失败: {str(e)}", "type": "upstream_error"}},
            )
        except Exception as e:
            logger.error("Web agent unexpected error: %s", e, exc_info=True)
            _mark_failed(acct, 503)
            raise HTTPException(
                status_code=503,
                detail={"error": {"message": f"意外错误: {str(e)}", "type": "upstream_error"}},
            )
...
async def proxy_chat_stream(
    ...
    if _is_web_agent(acct):
        ...
        try:
            ...
            logger.error("Web agent streaming HTTP error: %s", e, exc_info=True)
            _mark_failed(acct, 503)
            yield f"data: {json.dumps({'error': {'message': f'网页代理请求失败: {str(e)}', 'type': 'upstream_error'}})}\n\n"
            yield "data: [DONE]\n\n"
        except Exception as e:
            logger.error("Web agent streaming unexpected error: %s", e, exc_info=True)
            _mark_failed(acct, 503)
            yield f"data: {json.dumps({'error': {'message': f'意外错误: {str(e)}', 'type': 'upstream_error'}})}\n\n"
            yield "data: [DONE]\n\n"
        return
...
async def _test_account(acct: Account) -> dict:
    ...
    try:
        ...
        logger.warning("Test account HTTP error: %s", e)
        return {"success": False, "error": f"HTTP 错误: {str(e)}"}
    except Exception as e:
        logger.error("Test account unexpected error: %s", e, exc_info=True)
...
def get_accounts_status() -> list[dict]:
    ...
    for a in accounts:
        result.append({
            ...
            "cooldown_until": a.cooldown_until if a.on_cooldown else 0,
