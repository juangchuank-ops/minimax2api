...
- **🆕 智能区域检测** — 自动检测网络环境，国内用 .com，海外用 .io，避免连接问题
...
首次启动时，系统会自动检测你的网络环境并选择最优的 API 端点：

- 🇨🇳 **中国国内**: 自动使用 `api.minimaxi.com`（.com 域名国内可访问）
- 🌍 **海外/国际**: 自动使用 `api.minimax.io`（.io 域名国际可访问）
...
**故障排查**: 如果连接失败，检查日志中的 URL 切换建议，或访问 http://localhost:8000/api/region/detect 查看推荐配置。
...
| `GET /api/usage` | 使用统计 |
| `GET /api/region/detect` | 🆕 检测网络环境并返回推荐 base URL |
| `POST /api/region/validate` | 🆕 验证指定 base URL 是否可访问 |
