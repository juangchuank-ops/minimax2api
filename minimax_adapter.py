...
def _parse_jwt_user_id(jwt_token: str) -> str:
    ...
    try:
        ...
        logger.warning("Unexpected error parsing JWT token: %s", e)
        return ""
...
async def web_agent_chat_stream(
    ...
    try:
        ...
        logger.error("Streaming HTTP error: %s", e, exc_info=True)
        yield f"data: {json.dumps({'error': {'message': f'网页代理请求失败: {str(e)}', 'type': 'upstream_error'}})}\n\n"
    except Exception as e:
        logger.error("Streaming unexpected error: %s", e, exc_info=True)
...
async def test_adapter(jwt_token: str, real_user_id: str) -> dict:
    ...
    try:
        ...
        logger.error("Test adapter HTTP error: %s", e, exc_info=True)
        return {"success": False, "error": f"HTTP 错误: {str(e)}"}
    except Exception as e:
        logger.error("Test adapter unexpected error: %s", e, exc_info=True)
